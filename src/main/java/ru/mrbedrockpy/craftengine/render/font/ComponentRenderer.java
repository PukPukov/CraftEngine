...
import ru.mrbedrockpy.craftengine.util.lang.Style;
...
public final class ComponentRenderer {
    ...
    public float draw(TextBatch batch, Component root, float x, float y, int baseRGBA, Random rng) {
        if (root == null) return x;
        State st = new State(baseRGBA, rng != null ? rng : new Random(0));
        return drawComponent(batch, root, x, y, st);
    }

    public float drawString(TextBatch batch, String s, float x, float y, int rgba) {
    ...
    private float drawComponent(TextBatch b, Component c, float x, float y, State st) {
        Style s = c.getStyle();
    ...
    private float drawTranslatable(TextBatch b, String key, Object with, float x, float y, State st) {
    ...
    private float drawLiteral(TextBatch b, String lit, float x, float y, State st) {
    ...
    private float drawTextRun(TextBatch b, String s, float baseX, float yTop, State st) {
    ...
    private void drawSticker(TextBatch b, float x, float y, float w, float h,
