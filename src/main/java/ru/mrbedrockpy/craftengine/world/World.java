...
public class World {
    ...
    public Block getBlock(int x, int y, int z) {
        if (y < 0 || y >= Chunk.SIZE) return Blocks.AIR;
        Chunk chunk = getChunkByBlockPosition(x, z);
        if (chunk == null) return Blocks.AIR;
        return chunk.getBlock(Math.floorMod(x, Chunk.SIZE), y, Math.floorMod(z, Chunk.SIZE));
    }
    ...
    public boolean setBlock(int x, int y, int z, Block block) {
        ...
        return chunk.setBlock(Math.floorMod(x, Chunk.SIZE), y, Math.floorMod(z, Chunk.SIZE), block);
//        if (success) {
...
//        return success;
            ...
            for (int y = minY; y < maxY; y++) {
                for (int z = minZ; z < maxZ; z++) {
                    ...
                    if (block != Blocks.AIR) {
                        ...
                        if (aabb != null) boundingBoxList.add(aabb);
                    }
                }
            }
        }
    ...
    public List<AABB> allEntityAABBs() {
        ...
        for (Entity entity : this.entities) result.add(entity.getBoundingBox());
        return result;
    }
    ...
    public void removeEntity(Entity entity) {
        ...
        if (entity instanceof PlayerEntity player) players.remove(player);
    }

    public int getTopY(int x, int z) {
        for (int y = Chunk.SIZE; y >= 0; y--) {
            if (this.getBlock(x, y, z).isSolid()) return y;
        }
        return 0;
    }
    ...
    public BlockRaycastResult rayCast(Vector3f originF, Vector3f directionF, float maxDistance) {
        ...
        double sideDistX = stepX > 0 ? (Math.floor(pos.x) + 1.0 - pos.x) * deltaDistX : (pos.x - Math.floor(pos.x)) * deltaDistX;
        if (stepX == 0) sideDistX = Double.POSITIVE_INFINITY;

        double sideDistY = stepY > 0 ? (Math.floor(pos.y) + 1.0 - pos.y) * deltaDistY : (pos.y - Math.floor(pos.y)) * deltaDistY;
        if (stepY == 0) sideDistY = Double.POSITIVE_INFINITY;

        double sideDistZ = stepZ > 0 ? (Math.floor(pos.z) + 1.0 - pos.z) * deltaDistZ : (pos.z - Math.floor(pos.z)) * deltaDistZ;
