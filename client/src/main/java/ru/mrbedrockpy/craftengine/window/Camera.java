...
public class Camera implements ICamera {
    ...
    private static final float fov = CraftEngineConfiguration.MAIN_CONFIG.getFloat("fov", 70.0f);
    private static final float sprintFovDegrees = fov + 10 * CraftEngineConfiguration.MAIN_CONFIG.getFloat("fov.dinamic_multiplier", 1.0f);
    private static final float aspectRatio = 16f / 9f;
    private static final float zNear = 0.1f;
    private static final float zFar = 1000f;
    private static final double fovChangeStepRadians = Math.toRadians(Util.genericLerpStep(sprintFovDegrees, fov, 100));

    // STATE
    private boolean fovChangePhase = false;
    private double targetFovRadians = (float) Math.toRadians(fov);
