...
public final class AtlasManager implements AutoCloseable {
    ...
    public void uploadToShader(int program, String uniformArrayBaseName) {
        if (bindless) {
            for (int i = 0; i < atlases.size(); i++) {
                ...
                long h = atlases.get(i).getTexture().getHandle(); // см. пункт 2 про resident
            ...
            for (int i = 0; i < atlases.size(); i++) {
                ...
                glBindTexture(GL_TEXTURE_2D, atlases.get(i).getTexture().getId());
            }
    ...
    @Override
    public void close() {
        for (Atlas a : atlases) a.getTexture().close();
        atlases.clear();
    }
}